                ...
                <div class="product-box">
                  <div class="product-inner-box position-relative">
                    <div class="icons position-absolute">
                      <a href="http://localhost/wordShop/sieuthi_mini/client/info_product.html?id=${i.product_id}" class="text-decoration-none"><i class="fa-solid fa-eye"></i></a>
                    ...
                    <div class="cart-btn">
                      <button class="btn btn-white bg-white shadow-sm rounded-pill" data="${i.product_id}" data1="${i.name}" data2="${data.category_id}"><i class="fa-solid fa-cart-shopping"></i>Mua Ngay</button>
                    </div>
                  </div>
                  <div class="product-info">
                    ...
                    <div class="product-price">
                      <span>${parseFloat(i.price).toLocaleString(`de-DE`)} đ</span> 
                    </div>
                  </div>
                </div>
                </div>
                      ...
                      success: function (data) {
                        ...
                        <td data-th="Số lượng">
                         <div class="number_card">
                           <span class="minus_card" data="${data.price}" data2="${data.product_id}"><i class="fa-solid fa-minus"></i></span>
                           <input type="text" value="1" min="1"/>
                           <span class="plus_card" data="${data.price}" data2="${data.product_id}"><i class="fa-solid fa-plus"></i></span>
                         </div>
                        </td> 
                        <td id= "price-card" data-th="Tổng tiền" class="text-center text-danger card_total" data= "${data.price}" data2="${data.product_id}" data3=" ">${parseFloat(data.price).toLocaleString(`de-DE`)} đ</td> 
                        <td class="actions" data-th="">
                         <button class="btn btn-info btn-sm"><i class="fa fa-edit"></i>
                         </button> 
                         <button class="btn btn-danger btn-sm text-black btn-remove-card" data="${data.name}" data2="${data.product_id}"><i class="fa-solid fa-trash"></i>
                         </button>
                        </td>
                       </tr>
                        `;
                        ...
                        alert(` Sản phẩm ${data.name} đã được thêm vào giỏ hàng !`);
                          ...
                          $(`.minus_card`).click(function(){
                                ...
                                if(data.product_id == productID){
                                  let Element_price = $(this).parent().parent().parent().find(`#price-card`);
